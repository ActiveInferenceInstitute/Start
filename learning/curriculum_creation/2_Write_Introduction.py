...
def process_research_directory(
    client, research_dir: Path, fep_actinf_file: Path, output_dir: Path, dir_type: str
) -> tuple[int, int]:
    ...
    for i, research_file in enumerate(research_files, 1):
        try:
            logger.info(
                f"Processing {dir_type} research {i}/{len(research_files)}: {research_file.name}"
            )
            ...
            result = process_research_file(
                client, str(research_file), str(fep_actinf_file), str(output_dir)
            )
            if result:
                success_count += 1
