...
project_root = script_dir.parent.parent  # Go up two levels: curriculum_creation -> learning -> start
...
class CurriculumOrchestrator:
    ...
    def validate_inputs(self) -> bool:
        ...
        try:
            ...
            invalid_domains = [
                d for d in self.config.target_domains if d not in available_domains
            ]
            if invalid_domains:
    ...
    def run_domain_research_stage(self) -> bool:
        ...
        try:
            ...
            if self.config.target_domains:
                existing_domains = [
                    d for d in domains_to_process 
                    if d.get("name") in self.config.target_domains
                ]
            ...
            for domain in domains_to_process:
                ...
                try:
                    ...
                    if (
                        self.config.skip_existing_research and 
                        _domain_mod.check_domain_output_exists(domain_name, output_dir)
                    ):
    ...
    def run_entity_research_stage(self) -> bool:
        ...
        try:
            ...
            entities_to_process = _entity_mod.get_entities_to_process(
                entities_config,
                self.config.entity_priority_filter
            )
            
            # Filter to only target entities if specified, and add custom entities
            if self.config.target_entities:
                existing_entities = [
                    e for e in entities_to_process 
                    if e.get("name") in self.config.target_entities
                ]
                ...
                for entity_name in self.config.target_entities:
                    if entity_name not in config_entity_names:
                        ...
                        entity_description = (
                            getattr(self.config, '_custom_entity_description', '') or
                            f"Custom target audience: {entity_name}"
                        )
                        custom_entity = {
                            "name": entity_name,
            ...
            for entity in entities_to_process:
                ...
                try:
                    # Check if output already exists
                    if (self.config.skip_existing_research and 
                        _entity_mod.check_output_exists(entity_name, output_dir)):
...
"""
    ...
    def run_complete_pipeline(self) -> bool:
        ...
        """
        self.logger.info(
            "🚀 Starting Complete Active Inference Curriculum Generation Pipeline"
        )
...
def get_interactive_inputs() -> tuple[str, str, str, str]:
    ...
    try:
        ...
        available_domains = ["biochemistry", "neuroscience", "artificial_intelligence", "psychology"]
    ...
    print(f"Available domains: {', '.join(available_domains[:5])}{'...' if len(available_domains) > 5 else ''}")
    ...
    print(f"Available entities: {', '.join(available_entities[:5])}{'...' if len(available_entities) > 5 else ''}")
    ...
    print(f"Available languages: {', '.join(available_languages[:5])}{'...' if len(available_languages) > 5 else ''}")
    ...
    while True:
        ...
        if language in available_languages:
            ...
            if create_new in ['y', 'yes']:
                ...
                print("🔤 Note: Translation quality may vary for custom languages not in the AI model's training.")
                break
            else:
    ...
    if domain not in available_domains:
        print(f"   🆕 This is a custom domain")
    if language not in available_languages:
        print(f"   🆕 This is a custom language")
...
Examples:
  ...
  python generate_custom_curriculum.py --output-dir /path/to/custom/output
    ...
    parser.add_argument(
        ...
        nargs="+", 
        choices=[
            "domain-research", "entity-research", "curriculum", 
    ...
    # Performance options
    parser.add_argument(
        "--max-concurrent", type=int, default=3, help="Max concurrent API requests"
    )
    parser.add_argument(
    ...
    non_mode_args = [
        args.domains, args.entities, args.languages, args.output_dir, 
        args.overwrite, args.skip_existing, args.stages
    ]
    has_config_args = any(arg for arg in non_mode_args)
    
    should_run_interactive = (
        args.interactive or 
        (not args.non_interactive and not has_config_args)
    )
