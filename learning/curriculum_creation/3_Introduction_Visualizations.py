...
def extract_curriculum_metadata(curriculum_content: str) -> Dict[str, any]:
    ...
    objectives_pattern = (
        r"(?i)(?:learning\s+)?(?:objectives|outcomes|goals)[\s\S]*?(?=\n#+|\n\n|\Z)"
    )
...
def create_curriculum_metrics_chart(curricula_data: List[Dict], output_path: Path) -> None:
    ...
    try:
        ...
        width = 0.35
        axes[1, 0].bar(
            [i - width / 2 for i in x], df["code_block_count"], width, label="Code Blocks"
        )
        axes[1, 0].bar(
            [i + width / 2 for i in x],
            df["math_expressions_count"],
            width,
        ...
        axes[1, 1].hist(
            df["words_per_section"], bins=min(10, len(df)), edgecolor="black", alpha=0.7
        )
...
def create_domain_section_breakdown_chart(curriculum_data: Dict, output_path: Path) -> None:
    ...
    try:
        ...
        fig.suptitle(
            f"{entity_name} - Active Inference Curriculum Structure", fontsize=16, fontweight="bold"
        )
        ...
        cumulative_complexity = [
            i * (curriculum_data.get("words_per_section", 100)) for i in section_numbers
        ]
        ...
        plt.close("all")
        raise RuntimeError(
            f"Failed to create section breakdown chart for {entity_name}: {str(e)}"
        ) from e
...
def create_domain_complexity_chart(curriculum_data: Dict, output_path: Path) -> None:
    ...
    try:
        ...
        if non_zero_metrics:
            ax1.pie(
                non_zero_metrics.values(),
                labels=non_zero_metrics.keys(),
                autopct="%1.1f%%",
                startangle=90,
            )
        ...
        tech_values = [
            curriculum_data.get("code_block_count", 0),
            curriculum_data.get("math_expressions_count", 0),
        ]
        ...
        ax4.bar(
            structure_categories, structure_values, color=["gold", "silver", "#cd7f32"], alpha=0.7
        )
...
def create_domain_learning_objectives_chart(curriculum_data: Dict, output_path: Path) -> None:
    ...
    try:
        ...
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))
        fig.suptitle(
            f"{entity_name} - Learning Objectives Analysis", fontsize=16, fontweight="bold"
        )
        ...
        plt.close("all")
        raise RuntimeError(
            f"Failed to create learning objectives chart for {entity_name}: {str(e)}"
        ) from e
...
def create_domain_technical_content_chart(curriculum_data: Dict, output_path: Path) -> None:
    ...
    try:
        ...
        complexity_score = (
            ...
            + curriculum_data.get("math_expressions_count", 0)
            * 2  # Math expressions are moderately complex
        ...
        tech_density = (
            curriculum_data.get("code_block_count", 0)
            + curriculum_data.get("math_expressions_count", 0)
        ) / max(curriculum_data.get("section_count", 1), 1)
        ...
        plt.close("all")
        raise RuntimeError(
            f"Failed to create technical content chart for {entity_name}: {str(e)}"
        ) from e
