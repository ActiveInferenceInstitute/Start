...
class RepositoryManager:
    ...
    def get_summary(self) -> Dict[str, any]:
        ...
        total_size_mb = sum(status.get("size_mb", 0) for status in cloned.values())
        ...
        summary = {
            ...
            "by_category": {cat: len(repos) for cat, repos in categories.items()},
            "cloned_by_category": {},
        }

        # Count cloned repositories by category
        for cat in categories.keys():
            cloned_in_cat = sum(1 for repo in categories[cat] if repo in cloned)
