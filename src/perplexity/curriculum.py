...
def validate_curriculum_content(content: str, min_word_count: int = 100) -> Dict[str, Any]:
    ...
    if word_count < min_word_count:
        validation["warnings"].append(f"Content is short ({word_count} words, minimum {min_word_count})")
...
def save_complete_curriculum(output_dir: str, entity_name: str, sections: Dict[str, str]) -> Path:
    ...
    write_json(entity_dir / json_filename, {
        "timestamp": timestamp,
        "entity_name": entity_name,
        "sections": sections,
        "metadata": {"version": "1.0", "generation_date": datetime.now().isoformat(), "file_type": "complete_curriculum"}
    })
    return md_path


def process_research_file(client: OpenAI, research_file: str, fep_actinf_file: str, output_dir: str) -> Optional[Path]:
    ...
    try:
        ...
        for i, (section_name, content) in enumerate(sections.items(), 1):
            ...
            try:
                prompt = render_prompt("curriculum_section", {
