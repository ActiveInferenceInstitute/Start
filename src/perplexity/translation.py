...
def generate_translation_prompt(content: str, target_language: str) -> str:
    ...
    return render_prompt("translation", {
        "content": content,
...
def translate_curriculum(client: OpenAI, content: str, target_language: str, max_chunk_size: int = 4000, model: Optional[str] = None) -> str:
...
def process_translations(client: OpenAI, curriculum_dir: str, output_dir: str, target_languages: List[str]) -> tuple[int, int]:
    ...
    for curr_file in curriculum_files:
        ...
        for language in target_languages:
            ...
            existing = list(lang_dir.glob(f"{entity_name}_curriculum_{language.lower()}_*.md")) if lang_dir.exists() else []
            if existing:
                continue
            try:
