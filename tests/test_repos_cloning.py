...
class TestGetPredefinedRepositories:
    ...
    def test_predefined_repositories_structure(self):
        ...
        gnn = repos["gnn"]
        assert gnn.url == "https://github.com/ActiveInferenceInstitute/GeneralizedNotationNotation/"
        assert gnn.category == "active_inference"
        assert gnn.shallow is True

        cerebrum = repos["cerebrum"]
...
class TestCloneRepository:
    ...
    @patch("shutil.rmtree")
    @patch("pathlib.Path.mkdir")
    def test_clone_repository_destination_exists_with_force(
        self, mock_mkdir, mock_rmtree, mock_exists, mock_run
    ):
        ...
        with patch("pathlib.Path.rglob", return_value=[]):
            ...
            result = clone_repository(
                repo_info, force=True, progress_callback=progress_calls.append
            )
            ...
            with patch("pathlib.Path.exists", return_value=False):
                with patch("pathlib.Path.mkdir"):
                    with patch("pathlib.Path.rglob", return_value=[]):
                        ...
                        repo_info = RepoInfo(
                            "test-repo", "https://github.com/user/test-repo.git", branch="develop"
                        )
                        result = clone_repository(repo_info)
            ...
            with patch("pathlib.Path.exists", return_value=False):
                with patch("pathlib.Path.mkdir"):
                    with patch("pathlib.Path.rglob", return_value=[]):
                        ...
                        # Test shallow clone
                        repo_info = RepoInfo(
                            "test-repo", "https://example.com/repo.git", shallow=True
                        )
...
class TestCloneMultipleRepositories:
    ...
    def test_clone_multiple_repositories_with_progress(self, mock_clone, mock_get_repos):
        ...
        results = clone_multiple_repositories(
            ["test-repo"], progress_callback=progress_calls.append
        )

        assert len(results) == 1
        # Verify progress callback was passed to clone_repository
        mock_clone.assert_called_with(
            test_repo, force=False, progress_callback=progress_calls.append
        )
...
class TestCloneAllRepositories:
    ...
    def test_clone_all_repositories_no_category(self, mock_get_repos, mock_clone_multiple):
        ...
        mock_clone_multiple.assert_called_once_with(
            ["repo1", "repo2"], force=False, progress_callback=None
        )
    ...
    def test_clone_all_repositories_with_category(self, mock_get_repos, mock_clone_multiple):
        ...
        mock_clone_multiple.assert_called_once_with(
            ["repo1", "repo3"], force=False, progress_callback=None
        )
...
class TestCleanupFailedClones:
    ...
    @patch("pathlib.Path.iterdir")
    @patch("shutil.rmtree")
    def test_cleanup_failed_clones_success(
        self, mock_rmtree, mock_iterdir, mock_exists, mock_repo_root
    ):
...
class TestRepositoryCloningIntegration:
    ...
    def test_clone_repository_command_construction(self, mock_mkdir, mock_exists, mock_run):
        ...
        with patch("pathlib.Path.rglob", return_value=[]):
            ...
            repo_info = RepoInfo(
                "complex-repo", "https://github.com/user/repo.git", branch="develop", shallow=True
            )
            clone_repository(repo_info)
