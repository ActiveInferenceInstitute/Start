...
class TestRepositoryManager:
    ...
    def test_clone_repository_single(self, mock_clone_multiple):
        ...
        mock_clone_multiple.assert_called_once_with(
            ["test-repo"], force=False, progress_callback=None
        )
    ...
    @patch("shutil.which", return_value="/usr/bin/git")
    @patch(
        "shutil.disk_usage", return_value=(100 * 1024**3, 99 * 1024**3, 1024**2)
    )  # Very low free space
...
class TestFormatCloneResults:
    ...
    def test_format_clone_results_mixed(self):
        ...
        results = [
            CloneResult(
                "success-repo", True, Path("/clones/success"), size_mb=10.5, clone_time=30.2
            ),
            CloneResult("fail-repo", False, error_message="Network error"),
            CloneResult(
                "another-success", True, Path("/clones/another"), size_mb=5.0, clone_time=15.1
            ),
        ]
