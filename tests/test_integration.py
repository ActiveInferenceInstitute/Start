...
def test_makefile_help_target():
    ...
    result = subprocess.run(["make", "help"], cwd="src", capture_output=True, text=True, timeout=30)
...
def test_makefile_test_target_syntax():
    ...
    result = subprocess.run(
        ["make", "test", "--dry-run"], cwd="src", capture_output=True, text=True, timeout=30
    )
...
def test_makefile_lint_target_syntax():
    ...
    result = subprocess.run(
        ["make", "lint", "--dry-run"], cwd="src", capture_output=True, text=True, timeout=30
    )
...
def test_makefile_format_target_syntax():
    ...
    result = subprocess.run(
        ["make", "format", "--dry-run"], cwd="src", capture_output=True, text=True, timeout=30
    )
